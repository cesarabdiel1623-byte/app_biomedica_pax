...
void main() {
  group('Product Unit Tests - Getters & Basic Logic', () {
    test(
      'conditionLabel returns correct Spanish string based on productCondition',
      () {
        final p1 = Product(
          id: '1',
          sku: 'S1',
        ...
        final p3 = Product(
          ...
          requiresSerial: false,
          trackInventory: true,
          productCondition: 'remanufactured',
          createdAt: DateTime.now(),
        );
    ...
    test(
      'stockStatusLabel returns correct value based on current and minimum stock',
      () {
        final p1 = Product(
          id: '1',
          sku: 'S1',
  ...
  group('Product Unit Tests - JSON Parsing & Promotions', () {
    ...
    test(
      'Product.fromJson does not depend on the phone clock for promotions',
      () {
        final json = {
          'id': 'promo-server-clock',
          'sku': 'SKU-SERVER-CLOCK',
          'name': 'Producto promocional',
          'category': 'equipo_medico',
          'application': 'general',
          'unit_price_mxn': 1000.0,
          'cost_price_mxn': 600.0,
          'currency': 'MXN',
          'unit': 'pieza',
          'is_active': true,
          'requires_serial': false,
          ...
          'active_product_promotions': [
            {
              'id': 'prom-server-clock',
              'product_id': 'promo-server-clock',
              'discount_type': 'percentage',
              'discount_value': 20.0,
        ...
        expect(product.unitPriceMxn, 800.0);
        expect(product.oldPrice, 1000.0);
        expect(product.activePromotion?.endsAt, DateTime.utc(2000));
      },
    );
