...
void main() {
  group('T4.3.1 — RegistrationGateService Authoritative & Legacy Tests', () {
    test(
      '1. Authoritative seal: profile_completed=true + termsAccepted=false returns isComplete=true',
      () {
    ...
    test(
      '2. Authoritative seal: profile_completed=true + phone empty returns isComplete=true',
      () {
    ...
    test(
      '3. Fail-Closed: profile_completed=false + all fields claimed returns isComplete=false',
      () {
    ...
    test(
      '4. Incomplete/Test accounts: Unauthenticated and incomplete states evaluate to false',
      () {
    ...
    test(
      '5. Step 1 (Email): In-memory draft only, no Supabase Auth user created',
      () {
    ...
    test(
      '7. Step 3 (Phone): Advancing phone or skipping only updates in-memory draft',
      () {
    ...
    test(
      '8. Step 4 (Password): Setting password in memory does not trigger signUp',
      () {
        final draft = RegistrationDraft(
          email: 'paciente@gomedical.com',
          fullName: 'Carlos Mendoza',
    ...
    test(
      '9. New account registration: termsAccepted is strictly mandatory before signUp call',
      () {
        final draft = RegistrationDraft(
          email: 'paciente@gomedical.com',
          fullName: 'Carlos Mendoza',
    ...
    test(
      '10. Password security: Password is kept in memory only and cleared on dispose/cancel',
      () {
        final draft = RegistrationDraft(
          email: 'paciente@gomedical.com',
          fullName: 'Carlos Mendoza',
    ...
    test(
      '11. LoginScreen UI: Button "¿No terminaste tu registro?" is completely removed',
      () {
    ...
    test(
      '12. LoginScreen UI: Forgot Password, normal Login and Google Sign-In remain intact',
      () {
        final loginFile = File('lib/screens/auth/login_screen.dart');
        expect(loginFile.existsSync(), isTrue);
        final loginCode = loginFile.readAsStringSync();
    ...
    test(
      '14. T4.1 migration columns, grants and triggers remain fully active and compatible',
      () {
        final migrationFile = File(
          'supabase/migrations/20260814030000_t4_user_onboarding_gate.sql',
        );
    ...
    test(
      '15. T4.3 migration: does NOT falsify terms, does NOT modify business_name or has_app_access',
      () {
        final migrationT43 = File(
          'supabase/migrations/20260814040000_t4_3_backfill_legacy_completed_clients.sql',
        );
        ...
        expect(
          sql.contains(
            'EXISTS (SELECT 1 FROM public.client_addresses ca WHERE ca.client_id = c.id)',
          ),
          isTrue,
        );
