...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  Color _statusColor(String status) {
    switch (status) {
      ...
      case 'pending_review': return kOrange;
      case 'processing':
      ...
      child: Column(
        ...
        children: [
          const Text(
            ...
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: kNavy),
          ),
                ...
                children: List.generate(steps.length, (index) {
                  ...
                  return Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: isActive ? Colors.white : (isCompleted ? kGreen : Colors.white),
                      border: Border.all(
                        color: isCompleted ? kGreen : Colors.grey.shade300,
                        width: isActive ? 6 : 2,
                      ),
                      boxShadow: isActive ? [
                        BoxShadow(color: kGreen.withValues(alpha: 0.3), blurRadius: 6, spreadRadius: 1)
                      ] : null,
                    ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Row(
                        children: [
                          Icon(Icons.local_shipping_outlined, color: kNavy, size: 20),
                          SizedBox(width: 8),
                          Text('Dirección de Envío', style: TextStyle(fontWeight: FontWeight.bold, color: kNavy, fontSize: 14)),
                        ],
                      ),
                      const SizedBox(height: 8),
                      Text(
                      ...
                      const Row(
                        children: [
                          ...
                          SizedBox(width: 8),
                          Text('Notas / Instrucciones', style: TextStyle(fontWeight: FontWeight.bold, color: kNavy, fontSize: 14)),
                        ],
                      ),
                      const SizedBox(height: 8),
                      Text(
                                    ...
                                    children: [
                                      if (img != null)
                                        ClipRRect(
                                          ...
                                          child: Image.network(img, width: 50, height: 50, fit: BoxFit.cover, errorBuilder: (_, _, _) => Container(width: 50, height: 50, color: Colors.grey.shade100, child: const Icon(Icons.broken_image, size: 20))),
                                        )
                                      else
                                        Container(width: 50, height: 50, decoration: BoxDecoration(color: kPrimary.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.medical_services, color: kPrimary, size: 20)),
                                      const SizedBox(width: 12),
                                      Expanded(
                                        child: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            Text(item['product_name_snapshot'] ?? 'Producto biomédico', style: const TextStyle(fontWeight: FontWeight.bold, color: kNavy, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis),
                                            const SizedBox(height: 4),
                                            Text('Cantidad: $qty x ${formatCurrency(price)}', style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
                                          ],
                                        ),
                                      ),
                                      const SizedBox(width: 8),
                                      Text(formatCurrency(subtotalLine), style: const TextStyle(fontWeight: FontWeight.bold, color: kNavy, fontSize: 13)),
                                    ],
                                  ),
                                ),
                              );
              ...
              child: Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Resumen de Pago', style: TextStyle(fontWeight: FontWeight.bold, color: kNavy, fontSize: 14)),
                      const SizedBox(height: 12),
                      _summaryRow('Subtotal', formatCurrency(subtotal)),
                      const SizedBox(height: 6),
                      _summaryRow('IVA (16%)', formatCurrency(tax)),
                      const Padding(padding: EdgeInsets.symmetric(vertical: 8), child: Divider()),
                      _summaryRow('Total', formatCurrency(total), bold: true, size: 16),
                    ],
                  ),
                ),
              ),
