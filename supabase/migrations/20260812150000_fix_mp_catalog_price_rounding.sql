-- Migration: Preserve exact promotional pricing through SkyDropX and Mercado Pago.
-- Archivo local preparado para revisión. NO ejecutar db push sin autorización.
--
-- Seguridad:
-- - Flutter NO envía precios ni totales.
-- - La RPC recalcula productos, promociones vigentes, cupón, envío y total desde PostgreSQL.
-- - Los precios comerciales sin centavos son sólo presentación Flutter.
-- - Los cálculos financieros se redondean monetariamente a 2 decimales.
...
DECLARE
    ...
    v_financial_unit_price numeric(12,2);
BEGIN
    v_user_id := p_user_id;
    ...
    LOOP
        ...
        v_base_unit := GREATEST(v_item.unit_price_mxn, 0);

        SELECT LEAST(
            v_base_unit,
            COALESCE(MIN(GREATEST(0, app.promotional_price_mxn)), v_base_unit)
        )
        INTO v_effective_unit
        FROM public.active_product_promotions app
        WHERE app.product_id = v_item.product_id;

        v_financial_unit_price := ROUND(COALESCE(v_effective_unit, v_base_unit), 2);

        IF v_financial_unit_price <= 0 THEN
        ...
        v_effective_product_subtotal := v_effective_product_subtotal
            + ROUND(v_financial_unit_price * v_item.total_qty, 2);
    END LOOP;
            ...
            SELECT EXISTS (
                (
                    ...
                    SELECT
                        ...
                        MAX(ROUND(COALESCE(prom.effective_unit_price, GREATEST(p.unit_price_mxn, 0)), 2)) AS price
                    FROM public.cart_items c
                    JOIN public.products p ON p.id = c.product_id
                    LEFT JOIN LATERAL (
                        SELECT LEAST(
                            GREATEST(p.unit_price_mxn, 0),
                            MIN(GREATEST(0, app.promotional_price_mxn))
    ...
    IF NOT v_reused THEN
        ...
        LOOP
            ...
            INSERT INTO public.order_items (
                ...
                v_item.category, v_item.total_qty, v_financial_unit_price,
                ROUND(v_financial_unit_price * v_item.total_qty, 2)
            );
        END LOOP;
    END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'reused', v_reused,
        'pricing_source', 'postgres_active_product_promotions_financial_2dp'
    );
END;
$$;
