-- 20260813010000_t3_checkout_tax_fix.sql
...
BEGIN
    ...
    LOOP
        ...
        v_cart_fingerprint := v_cart_fingerprint || 
            ...
            TO_CHAR(v_commercial_unit::numeric, 'FM9999999990.000000') || ':' ||
    ...
    LOOP
        ...
        v_subtotal := v_subtotal + ROUND(v_commercial_unit * v_item.total_qty, 2);
    END LOOP;

    -- Total: los productos ya incluyen IVA. Solo se extrae el informativo y el envío sí se suma.
    v_tax := ROUND(v_subtotal - (v_subtotal / 1.16), 2);
    v_total := ROUND(GREATEST(v_subtotal + v_quote.customer_shipping_amount, 0), 2);
                        ...
                        MAX(
                            COALESCE(
                                LEAST(
                                    ...
                                    (SELECT MIN(ROUND(GREATEST(0, promotional_price_mxn), 0)) FROM public.active_product_promotions WHERE product_id = p.id)
                                ),
                                ROUND(GREATEST(p.unit_price_mxn, 0), 0)
                            )
                        ) AS price
